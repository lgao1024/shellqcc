#!perl
#this script transform xsd file to qe input file, just set filename of xsd. but remerber to alter relative input parameters in qe.txt
#ponychen; lgao
#250326
#email:18709821294@outlook.com
#email:lgao1024@outlook.com
...
my $filename;
eval {
   $filename  = Documents->ActiveDocument->Name;
};
if ($@) {
   $filename = "pwscf";  # without the .xsd extension
}
my $doc = $Documents{"$filename.xsd"};
my $pos = Documents->New("$filename.txt");       #output txt name
